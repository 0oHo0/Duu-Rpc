...
public class VertxTcpClient {
     ...
     * @description: 发送Tcp请求
     * @author: duu
     * @date: 2024/3/25 21:00
     * @param: rpcRequest
     * @param servicePort
     * @param serviceHost
     * @return: RpcResponse
     **/
    public static RpcResponse doRequest(RpcRequest rpcRequest, Integer servicePort, String serviceHost) throws InterruptedException, ExecutionException {
        // 发送 TCP 请求
        Vertx vertx = Vertx.vertx();
        NetClient netClient = vertx.createNetClient();
        CompletableFuture<RpcResponse> responseFuture = new CompletableFuture<>();
        netClient.connect(servicePort, serviceHost,
                result -> {
                    if (!result.succeeded()) {
                        System.err.println("Failed to connect to TCP server");
                        return;
